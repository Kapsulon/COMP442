...
namespace lang
{
    ...
    private:
            ...
            },
            {
                ASTNode::Kind::Class, [&](GENERATOR_ARGS) {
                    ...
                    if (std::none_of(ast->children.begin(), ast->children.end(), [](const std::shared_ptr<const ASTNode> &child) { return child->kind == ASTNode::Kind::InheritList; })) {
                        node->table.emplace_back(SymbolTableNode{
        ...
        std::optional<SymbolTableNode> generateSymbolTable();
        std::optional<SymbolTableNode> applyGenerator(SymbolTableNode *parent, std::shared_ptr<const ASTNode> ast);
